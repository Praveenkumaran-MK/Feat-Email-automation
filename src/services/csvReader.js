...
/**
 * Get all teachers including invalid ones (for failure reporting)
 * @param {string} filePath
 * @returns {Array} Array of {valid, teacher, error}
 */
export function getAllTeachersWithValidation(filePath) {
  try {
    const fileContent = readFileSync(filePath, "utf-8");
    const records = parse(fileContent, {
      columns: true,
    ...
    records.forEach((record, index) => {
      ...
      results.push({
        ...
        teacher: validation.teacher || {
          section: record.section?.trim().toLowerCase() || "unknown",
          name: record.teacher_name?.trim() || "Unknown",
          email: record.emailid?.trim().toLowerCase() || "invalid",
        },
...
export function validateStudent(record, lineNumber) {
  ...
  if (!record.emailid || !validateEmail(record.emailid))
    errors.push(`Line ${lineNumber}: Invalid emailid (${record.emailid})`);

  // regno[format is 24CSXXX]
  const regnoRegex = /^24CS\d{3}$/i;
  if (!record.regno || !regnoRegex.test(record.regno)) {
    errors.push(
      `Line ${lineNumber}: Invalid regno format (${record.regno}). Should be 24CSXXX`,
    );
  }
  ...
  if (!record.section || !sectionRegex.test(record.section)) {
    errors.push(
      `Line ${lineNumber}: Invalid section (${record.section}). Should be a-o`,
    );
  }
...
export function validateTeacher(record, lineNumber) {
  ...
  if (!record.teacher_name)
    errors.push(`Line ${lineNumber}: Missing teacher name`);
  if (!record.emailid || !validateEmail(record.emailid))
    errors.push(`Line ${lineNumber}: Invalid emailid (${record.emailid})`);
...
export function validateSubscriber(record, lineNumber) {
  ...
  return {
    ...
    subscriber: {
      ...
      subscribed_date:
        record.subscribed_date || new Date().toISOString().split("T")[0],
